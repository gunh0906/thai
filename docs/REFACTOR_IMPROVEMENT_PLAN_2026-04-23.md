...
- Deploy closure
  ...
  - 관리형 AI 연결 런타임을 재배포했고 endpoint와 runtime identifier는 문서에 기록하지 않는다.
...
## 실행 상태 추가 업데이트 (2026-04-24 security/search batch)

- Security check
  - 서버 측 AI key 저장소가 secret-only 상태임을 확인했다. 실제 secret name/value는 문서에 기록하지 않는다.
  - No local `.dev.vars` file exists under `workers/openai-assist`.
  - `.dev.vars` and `.wrangler/` are ignored by `.gitignore`; only `.dev.vars.example` is tracked.
  - Full git history scan for real `sk-...` key patterns returned `MATCH_COUNT=0`.
- Local search data
  - Added 20 production / equipment / company-life vocab entries and matching representative sentences.
  - Covered production plan, work standard, daily report, lot number, quantity, inventory, rework, scrap, defect cause, corrective action, equipment inspection, lubricant, coolant, pressure, temperature, measuring tool, vernier caliper, torque wrench, approval pending, and meeting minutes.
- Search filter
  - Specific worksite object intents now demote or remove generic generated vocab when concrete local terms should win.
  - `폐기` and `조치` now win top1 for the representative production-floor queries.
- Verification
...
- Next checkpoint
  - Push `20260424b`, then verify live GitHub Pages HTML points to `app.js?v=20260424b`.
